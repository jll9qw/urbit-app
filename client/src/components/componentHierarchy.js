	...
	App: {
		...
		Header: {
			...
			Nav: {
				...
				logIn: credentials => {
					/*take the returned data and query db.Users for all of the user data......then return that userData back to the state...*/ then(
						user => this.setState({ userData: user })
					);
				},
				subscribe: data => {
					/*take returned data and create a new user in db.Users...then return that new userData back to the state...*/ then(
						newUser => this.setState({ userData: newUser })
					);
				},
				LogInModal: {
